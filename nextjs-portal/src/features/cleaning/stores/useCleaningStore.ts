...
const useCleaningStore = create<CleaningState>()(
  persist(
    (set, get) => ({
      tasks: [],
      logs: [],
      ...
      addTask: (task) => {
        set((state) => ({
          tasks: [...state.tasks, { ...task, id: uuidv4(), lastDoneAt: undefined }],
        }));
      },
      ...
      markTaskComplete: (taskId, taskTitle, doneBy) => {
        ...
        set((state) => ({
          ...
          tasks: state.tasks.map((task) =>
            task.id === taskId ? { ...task, lastDoneAt: now } : task
          ),
        }));
      },
      ...
      getTasksForToday: () => {
        ...
        return get().tasks.filter((task) => {
          ...
          switch (task.interval.type) {
            ...
            case 'weekly':
              // Check if it's the specified weekday and not done this week
              if (task.interval.weekday !== undefined) {
                ...
                if (currentDay === targetDay) {
                  // If today is the target day, check if it was done this week
            ...
            case 'everyN':
              if (task.interval.n) {
                if (!lastDoneDate) return true; // If never done, it's due
    ...
    {
      name: 'cleaning-storage', // unique name
